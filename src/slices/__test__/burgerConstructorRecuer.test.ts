...
describe('Тест экшенов конструктора бургеров', () => {
  const startState: TConstructorSlice = JSON.parse(
  ...
  test('Тест экшена добавления ингредиента', () => {
    ...
    const endState: TConstructorSlice = JSON.parse(JSON.stringify(startState));
    endState.constructorItems.ingredients.push({
      ...ingredient,
      id: 'mockedID'
    });

    const newState = constructorReducer(startState, addIngredients(ingredient));
  ...
  describe('Тест экшенов изменения порядка ингредиентов', () => {
    test('Тест экшена перемещения ингредиента вверх', () => {
      const ingredientId = 1;
      const endState: TConstructorSlice = JSON.parse(
    ...
    test('Тест экшена очистки конструктора', () => {
      const endState: TConstructorSlice = {
        ...startState,
        constructorItems: {
          bun: null,
          ingredients: []
